...
先确认已经在普通 PowerShell 中登录 GitHub CLI：
...
```text
https://github.com/你的用户名/hotspot-radar-mvp
```
...
如果 `git push` 无法连接 GitHub，脚本会自动回退到 `publish-github-api.ps1`，通过 GitHub API 上传当前已跟踪文件。
...
```text
Linux VPS / Oracle Cloud Always Free
  + Node.js 20+
  + PM2
  + Nginx
  + HTTPS
...
- 这是轻量 MVP，不包含完整登录系统。
- 如果不开启公开只读模式，公开访问者可能修改关键词、来源和保留时间。
- 生产环境建议开启 `PUBLIC_READONLY=1` 并设置足够长的 `ADMIN_TOKEN`。
- 浏览器通知通常需要 HTTPS 才能稳定使用。
